...
interface AccountBalanceAuditProps {
  accountId: number;
}
...
export default function AccountBalanceAudit({ accountId }: AccountBalanceAuditProps) {
  ...
  const fetchAuditTrail = async (offset: number = 0, append: boolean = false) => {
    try {
      ...
      const response = await fetch(`/api/accounts/${accountId}/audit?limit=${pageSize}&offset=${offset}`);
